...
def main():
    ...
    train_subset, val_subset = random_split(train_data, [int(len(train_data) * 0.8), int(len(train_data) * 0.2)])
    ...
    for epoch in range(number_epoch):
        ...
        for train_data, train_label in train_loader:
            ...
            sum_correct_train_prediction += torch.sum(train_prediction == train_label.to(device)).item()
        ...
        for val_data, val_label in val_loader:
            ...
            sum_correct_val_prediction += torch.sum(val_prediction == val_label.to(device)).item()
